...
watch(repository, (newRepo: RepositoryDetail) => {
  ...
  Object.assign(state, {
    ...
    serviceUrl: newRepo.serviceUrl || '',
    entityIds: newRepo.entityIds?.length ? [...newRepo.entityIds] : [''],
    spConnectorId: newRepo.spConnectorId || '',
    active: newRepo.active ?? false,
    created: date ? dateFormatter.format(date) : '',
  })
}, { immediate: true })
  ...
  <div class="max-w-200 m-auto">
    ...
    <UCard>
      ...
      <RepositoryForm
        v-model="state"
        :mode="mode"
        @submit="(data) => onSubmit(data as RepositoryUpdatePayload)"
        @cancel="onCancel"
      />
    </UCard>
  </div>
