...
@roles_required("system_admin", "repository_admin")
@validate(response_by_alias=True)
def id_patche(group_id: str, body: MemberPatchRequest) -> tuple[GroupDetail | ErrorResponse, int]:
```
...
| -------- | ------------------------------------------------------- | ---------------------- |
| group_id | str                                                     | グループ ID            |
| body     | [`MemberPatchRequest`](07.schema.md#memberpatchrequest) | 追加 /除外するユーザー |
...
1. `group_id` を引数に [`services.permmission:filter_permitted_group_ids`](../05.services/08.permissions.md#filter_permitted_group_ids) を呼び出し、更新可能か確認する
   - 更新可能でなければ `ErrorResponse` オブジェクトを作成し、ステータスコード 403 とともに返す。
2. 引数 `group_id`,`body` の `add`, `remove` 属性をもとに、 [`services.groups:update_member`](../05.services/03.groups.md#update_member) を呼び出し、グループ情報を更新する。
3. グループが存在しない場合、 `ErrorResponse` オブジェクトを作成し、ステータスコード 404 とともに返す。
4. 更新内容に競合があった場合、 `ErrorResponse` オブジェクトを作成し、ステータスコード 409 とともに返す。
5. グループ更新処理に失敗した場合、 `ErrorResponse` オブジェクトを作成し、ステータスコード 500 とともに返す。
6. 更新されたグループ情報を `GroupDetail` オブジェクトとして、ステータスコード 200 とともに返す。
...
2. [`service.groups:get_filter_options`](../05.services/03.groups.md#get_filter_options) でフィルターオプションを取得
3. FilterOption を返却
