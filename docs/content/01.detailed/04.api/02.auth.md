...
7. is_member_of を引数に [`services.permissions:get_user_roles`](../05.services/08.permissions.md#get_user_roles) で権限を確認し "system_admin", "repository_admin" が含まれていなければ 403 を返却する
8. eppn,is_member_of, user_name から LoginUser オブジェクトを作成
9.  [`LoginUser`](../03.entities/12.login-user.md#loginuser) オブジェクトを引数に `flask_login.login_user` を行い認証情報の登録をおこなう
10. `flask.session` から `_id` をキーにセッション ID を取得し [`LoginUser`](../03.entities/12.login-user.md#loginuser) の [`_session_id_`](../03.entities/12.login-user.md#属性) に登録
11. Redis にsession_id をキーに eppn,login_date,is_member_of を登録する
12. クエリパラメータに next があれば next を付加しトップページにリダイレクトする
