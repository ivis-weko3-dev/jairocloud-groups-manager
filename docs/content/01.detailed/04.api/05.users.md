...
3. `UserSearchCriteria` オブジェクトをもとに、`services.users:search` を呼び出してユーザーの検索結果を取得する。
...
| tuple\[ErrorResponse, Literal\[409]]               | ユーザーIDまたはePPNがすでに存在する場合のエラーレスポンス |
| tuple\[ErrorResponse, Literal\[500]]               | ユーザー作成処理に失敗した場合のエラーレスポンス           |

#### 処理内容

1. リクエストコンテキストからログインユーザー情報を取得する。
2. ユーザーID、ePPNがすでに存在するかを確認する。  
...
4. 引数 `body` をもとに、`services.users:create` を呼び出してユーザーを作成する。
...
**処理内容**
1. リクエストコンテキストからログインユーザー情報を取得する。
2. 引数 `user_id` をもとに、`services.users:get_by_id` を呼び出してユーザー情報を取得する。
...
3. 引数 `body` をもとに、`services.users:update` を呼び出してユーザー情報を更新する。
...
#### 処理内容
1. 引数 `user_id` をもとに、`services.users:delete` を呼び出してユーザーを削除する。
