...
| 名前      | 型       | 説明                       |
| --------- | -------- | -------------------------- |
| group_ids | set[str] | 削除するグループのidリスト |
...
#### 処理内容
1. group_idsをもとに`method`="DELETE",`path`="/Groups/{id}"でlist\[[BulkOperation](../03.entities/11.bulk-request.md#bulkoperation)] を作成する。
...
2. [`token.get_client_secret`](../05.services/01.token.md#get_client_secret)でクライアントシークレットを取得する。
3. 認証情報取得時に[`OAuthTokenError`](../02.foundation/07.exc.md#oauthtokenerror),[`CredentialsError`](../02.foundation/07.exc.md#credentialserror)が発生した場合はそのまま送出する。
4. group_idをもとに[`client.groups:get_by_id`](../06.clients/03.groups.md#get_by_id)を呼び出し、詳細情報から現在所属しているメンバーリストを確認する。
5. addに現在所属しているユーザーIDが含まれていれば取り除く。
6. removeに現在所属していないユーザーID，サーバー設定値のシステム管理者IDが含まれていれば取り除く。
7. removeのメンバーを取り除いても所属するメンバーが0にならないことを確認し0になる場合はaddにシステム管理者IDを追加する。
8. `if add & remove`がtrueならば[RequestConflict](../02.foundation/07.exc.md#requestconflict)を送出する。
9.  [`build_update_member_operations`](../05.services/07.utils.md#build_update_member_operations)でlist\[PatchOperation]を作成する。
10. `include`に`"members"`を設定し、[`client.groups:patch_by_id`](../06.clients/03.groups.md#patch_by_id)を呼び出す。
11. レスポンスが[MapError](../03.entities/04.map_error.md)の場合[ResourceInvalid](../02.foundation/07.exc.md#resourceinvalid)を送出する。
12. 更新されたグループ情報を[`GrouprDetail`](../03.entities/09.group-detail.md)オブジェクトにマッピングする。
13. 更新されたグループ詳細情報オブジェクトを返す。
